...
设计日期：2026-09-10。第一版已部署为 VPS 静态页面；2026-09-12 的暂停、密集下蛋和资源回收改动见 [性能验证](performance-2026-09-12.md)。下文仍包含待真机验证的设计目标。
...
- 暂停时停止刷新循环与装饰动画；恢复时扣除暂停时间。窗口变化由独立尺寸观察器触发一次重绘，不恢复游戏时钟。
- HUD 按分数和设置变化通知，实体快照在同一状态下复用；一次下蛋共用占用索引完成全部位置预留，网格仅在窗口尺寸变化时重建。
...
| 瞬时粒子与蛋壳 | 普通装饰 40 个；满额时省略装饰，保留游戏实体。庆祝另保留 51 个对象，同一时间只播放一组 |
| 蛋孵化 | 5.46 秒；裂纹分别在 2.70 与 4.02 秒出现 |
| 画布分辨率 | DPR 初始限制为 2，并给渲染缓冲区设约 400 万像素预算 |
| 画质回退 | 先减少粒子和分辨率；目标设备需要时采用 30 fps 显示 |
| 声音并发 | 分类型复用最多 8 个短音效播放器，背景音乐 1 个；同一固定步批量孵化只请求一次破壳声 |

角色显示按实体 ID 复用，离场时销毁；瞬时装饰完成后销毁并受总数预算约束。页面重启一局不重复注册输入监听、不重复加载纹理。小鸡用简单方向与边界规则，不使用群体全配对避碰或物理引擎。PNG/WebGL 纹理内存与像素数有关，SVG 下载只有几 KB 也不代表纹理内存为零。
